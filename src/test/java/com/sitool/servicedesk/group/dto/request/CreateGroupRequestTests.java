...
public class CreateGroupRequestTests {
    ...
    void  groupNameIsNullValidationFailedWithMessage() {
        ...
        CreateGroupRequest request = new CreateGroupRequest(null, "Some description.", userIds);
    ...
    void  groupNameIsBlankValidationFailedWithMessage() {
        List<UUID> userIds = new ArrayList<>();
        CreateGroupRequest request = new CreateGroupRequest("", "Some description.", userIds);
        Set<ConstraintViolation<CreateGroupRequest>> violations = validator.validate(request);
        String expectedMessage = messages.getString("group.name.notBlank");
    ...
    void  groupNameHasLessThanMinLengthValidationFailedWithMessage() {
        List<UUID> userIds = new ArrayList<>();
        CreateGroupRequest request = new CreateGroupRequest("A", "Some description.", userIds);
        Set<ConstraintViolation<CreateGroupRequest>> violations = validator.validate(request);
    ...
    void  groupNameHasMoreThanMaxLengthValidationFailedWithMessage() {
        ...
        CreateGroupRequest request = new CreateGroupRequest(groupName, "Some description.", userIds);
        Set<ConstraintViolation<CreateGroupRequest>> violations = validator.validate(request);
        String expectedMessage = messages.getString("group.name.length");
        assertThat(violations).hasSize(1);
        assertThat(violations)
